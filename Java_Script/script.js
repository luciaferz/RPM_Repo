function highlightActivePage() { //marcar en el menú qué página se esta viendo en el momento.
  ...
  $(".Menu a").each(function () { //Recorre todos los enlaces del menú y usa jQuery para seleccionar todos los <a> del menú.
    const $link = $(this);
...
$(function () {
  ...
  if ($track.length) {
    ...
    $track.on("mouseleave", function () { //Reanuda el movimiento del slider al salir el ratón.
  ...
  function getFieldMessage($field) {
    ...
    if (fieldId === "nombre" && !value) { //Hace obligatorio rellenar el campo
  ...
  $contactForm.on("submit", function (event) {
    ...
    $requiredFields.each(function () { //Recorre cada campo obligatorio para validar su estado.
      const $field = $(this);
      updateFieldState($field);//Valida el campo y actualiza su estado visual (borde rojo y mensaje de error).

      if (getFieldMessage($field)) { //Comprueba si hay error y lo añade a la lista de campos inválidos.
